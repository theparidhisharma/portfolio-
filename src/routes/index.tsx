...
function ChapterOne() {
  ...
  const ghostY = useTransform(scrollYProgress, [0, 0.2], [0, 160]);
  const ghostRotate = useTransform(scrollYProgress, [0, 0.2], [0, 8]);
  const veil = useTransform(scrollYProgress, [0, 0.12], [1, 0]);
      ...
      <motion.div className="relative" style={{ y }}>
        ...
        <Fade delay={1.3}>
          <p className="lede mt-12 max-w-[36rem]">
            Software engineer building distributed systems, predictive
            intelligence, and interfaces built to be remembered.
          </p>
        </Fade>
        <Fade delay={1.5}>
          <dl className="mt-12 grid grid-cols-2 gap-y-4 rule-t pt-8 text-sm text-muted-foreground md:grid-cols-4">
            <div>
              ...
              <dd className="mt-2">Delhi, India</dd>
            </div>
            <div>
              <dt className="eyebrow">Currently</dt>
              <dd className="mt-2">Flipkart</dd>
            </div>
            <div>
              <dt className="eyebrow">Focus</dt>
              <dd className="mt-2">Distributed systems</dd>
            </div>
            <div>
              <dt className="eyebrow">Features</dt>
              <dd className="mt-2">{String(WORKS.length).padStart(2, "0")}</dd>
            </div>
          </dl>
        </Fade>
        <Fade delay={1.7}>
          <div className="mt-[8vh] flex items-center gap-6">
            ...
            />
            <span className="marker">Scroll to begin</span>
          </div>
        </Fade>
      </motion.div>
  ...
  return (
    <section id="journal" className="scroll-mt-24 px-[6vw] py-[16vh]">
      ...
      <h2 className="display mb-[8vh] max-w-[24ch] text-[11vw] leading-[0.88] md:text-[5.4vw]">
        <SplitLine text="Notes from the workshop" stagger={0.1} />
      </h2>
  ...
  return (
    <section id="dispatches" className="scroll-mt-24 px-[6vw] py-[14vh]">
      ...
      <h2 className="display mb-[7vh] text-[11vw] leading-[0.88] md:text-[5vw]">
        <SplitLine text="Said elsewhere" stagger={0.12} />
      </h2>
      <LinkedInWall />
    </section>
  );
...
function WorkPlate({ work, position }: { work: (typeof WORKS)[number]; position: number }) {
  ...
  const { mode } = useTheme();
  const image = getFeatureImage(work.slug, mode);
